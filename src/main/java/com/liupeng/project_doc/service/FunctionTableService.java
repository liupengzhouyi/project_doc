package com.liupeng.project_doc.service;

import com.liupeng.project_doc.entity.FunctionTable;

import java.util.List;

/**
 * 函数表(FunctionTable)表服务接口
 *
 * @author liupeng
 * @since 2021-04-20 20:37:35
 */
public interface FunctionTableService {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    FunctionTable queryById(Integer id);

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit  查询条数
     * @return 对象列表
     */
    List<FunctionTable> queryAllByLimit(int offset, int limit);

    /**
     * 查询所有数据
     *
     * @return 对象列表
     */
    List<FunctionTable> queryAll();

    /**
     * 新增数据
     *
     * @param functionTable 实例对象
     * @return 实例对象
     */
    FunctionTable insert(FunctionTable functionTable);

    /**
     * 修改数据
     *
     * @param functionTable 实例对象
     * @return 实例对象
     */
    FunctionTable update(FunctionTable functionTable);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    boolean deleteById(Integer id);

}
